          ...
          if(value > 0){
            await Transactions.create({
              ...
              await Accounts.update({balance: parseFloat(account.balance) - value}, {
                ...
                console.log(err);
                res.status(400).send({message: 'Ocorreu um erro, a transação não foi concluida!'})
              })
            })
          }else{
            res.status(200).send({message:'Valor da transação não pode ser negativo ou zerado!'});
          }
        }else{
